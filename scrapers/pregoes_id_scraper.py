...
class PregoesIdScraper:
    """
    Consulta pregão por id_compra no módulo legado:
      GET https://dadosabertos.compras.gov.br/modulo-legado/3.1_consultarPregoes_Id
    Parâmetros:
      - id_compra (obrigatório)
      - dt_alteracao (opcional)
    """
    def __init__(self):
        self.base_url = "https://dadosabertos.compras.gov.br/modulo-legado/3.1_consultarPregoes_Id"
    ...
    def _map_pregao(self, rec: Dict, palavra_chave: Optional[str]) -> Dict:
        ...
        modalidade = rec.get("nome_modalidade") or rec.get("modalidade") or "PREGÃO"
        descricao = rec.get("objeto") or rec.get("informacoes_gerais") or rec.get("descricao") or ""
        titulo = rec.get("titulo") or (descricao[:120] if descricao else f"Pregão {numero}")
        status = rec.get("situacao_aviso") or rec.get("situacao_pregao") or rec.get("status") or ""
    ...
    def buscar(self, id_compra: str, palavra_chave: Optional[str] = None, dt_alteracao: Optional[str] = None) -> List[Dict]:
        ...
        try:
            ...
            for rec in rows:
                try:
                    resultados.append(self._map_pregao(rec, palavra_chave))
                except Exception:
                    continue
            return resultados
        except Exception as e:
            print(f"Erro ao consultar pregão por ID (legado): {e}")
            return []
